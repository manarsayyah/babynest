 ...
 *   - not mapped: leaves its existing images alone. Only if it has no image at all does it add one BabyNest
 *     placeholder so the UI never has an empty gallery.
 *
...
import { hasPhoto, placeholderImageUrl, productImageUrl } from "./product-images"

const DRY = process.argv.includes("--dry")

async function main() {
  const mongoose = await connectToDatabase()
  const counts = { photoProducts: 0, placeholderProducts: 0, baseUpdated: 0, extraRemoved: 0, variantImagesUpdated: 0, placeholdersAdded: 0 }
  try {
    ...
    for (const product of products) {
      ...
      if (hasPhoto(product.slug)) {
        ...
        const extras = await ProductImage.find({ productId, variantId: null, displayOrder: { $gte: 1 }, imageUrl: /^https:\/\/placehold\.co\// }).select("_id").lean()
        ...
        if (!anyImage) {
          ...
          if (!DRY) {
            await ProductImage.create({ productId, imageUrl: placeholderImageUrl(product.name, 0), altText: product.name, displayOrder: 0, isPrimary: true, variantId: null })
          }
        }
      }
    }
